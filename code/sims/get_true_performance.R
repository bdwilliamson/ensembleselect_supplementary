...
get_true_performance <- function(test_dat, linear = TRUE, family = "binomial", beta, x_dist = "normal") {
  if (linear) {
    linear_predictor <- cbind(1, as.matrix(test_dat %>% select(-y)))%*%beta
  } else {
    linear_predictor <- nl_conditional_mean(test_dat %>% select(-y), beta, x_dist)
  }
