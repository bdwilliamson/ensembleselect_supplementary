...
lib_paths <- .libPaths()
# variable importance
# if you need to update: remotes::install_github("bdwilliamson/vimp", upgrade = "never")
library("vimp")
# variable selection
# if you need to update: remotes::install_github("bdwilliamson/flevr@main", upgrade = "never")
library("flevr")
library("stabs")
library("knockoff")
library("parallel")
...
if (this_analysis$covariates == 0) {
  dataset <- dataset %>%
    select(institution, !!this_analysis$outcome, starts_with("lab1"),
           starts_with("lab2"), starts_with("lab3"), starts_with("lab4"),
           starts_with("lab5"), starts_with("lab6"), starts_with("cea"))
}
...
all_algos <- c("lasso", "lasso-SS", "lasso-KF", "SL", "SL-SS", "SPVIM", "SPVIM-RR")
...
system.time(
    selection_results <- select_from_all(imputed_training_data = imputed_datasets,
...
if (is.list(pooled_set)) {
    ...
    final_selected_vars_lst <- names(analysis_dataset %>%
                                     select(-id, -!!this_analysis$outcome))[pooled_set]
